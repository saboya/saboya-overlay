...
NV_TOOLS_PV=${PV}
AMD_64_URI="${NV_URI}Linux-x86_64/${PV}/${AMD64_NV_PACKAGE}.run"

NV_DEV_URI="https://developer.nvidia.com/vulkan-beta-$(ver_cut 1- ${PV})-linux"
NV_URI="https://us.download.nvidia.com/XFree86/"

if [[ ! -z $(ver_cut 3 ${PV}) ]]; then
	AMD64_URI="${NV_DEV_URI} -> ${AMD64_NV_PACKAGE}.run"
fi

if [[ ${PV} == "440.66.12" ]]; then
	NV_TOOLS_PV="440.82"
fi
...
src_compile() {
	...
	if use tools; then
		emake -C "${S}"/nvidia-settings-${NV_TOOLS_PV}/src/libXNVCtrl \
			DO_STRIP= \
			LIBDIR="$(get_libdir)" \
			NVLD="$(tc-getLD)" \
			NV_VERBOSE=1 \
